...
class SequentialMemory(Memory):
    ...
    def __init__(self, limit, memory_size=None, **kwargs):
    ...
    def sample(self, batch_size, batch_idxs=None):
        ...
        if batch_idxs is None:
            ...
            if self.memory_size:
                low = max(self.window_length, self.nb_entries - 1 - np.prod(self.memory_size))
            else:
                low = self.window_length
            batch_idxs = sample_batch_indexes(
                low, self.nb_entries - 1, size=batch_size)
